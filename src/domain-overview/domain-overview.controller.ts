...
export class DomainOverviewController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Content topics retrieved successfully',
  })
  @ApiResponse({ status: 400, description: 'Invalid domain or parameters' })
  @ApiQuery({
    name: 'limit',
  ...
  @ApiOperation({
    summary: 'Get domain authority metrics from multiple sources',
  })
  @ApiResponse({
    status: 200,
    description: 'Domain authority metrics retrieved successfully',
  })
  ...
  // The general :domain route should come last to avoid conflicts
  @Get(':domain')
  @ApiOperation({
    summary: 'Get comprehensive domain overview and authority metrics',
  })
  @ApiResponse({
    status: 200,
