...
export class DomainOverviewService {
  ...
  async getDomainOverview(
    ...
    const topCountries: CountryTraffic[] = countries
      .slice(0, 5)
      .map((country, index) => {
        ...
        return {
          country,
          traffic: Math.floor(
            (Math.random() * 100000 + 10000) * (percentage / 100),
          ),
  ...
  async getTopKeywords(
    dto: DomainTopKeywordsDto,
  ): Promise<DomainTopKeywordsResponse> {
  ...
  async getDomainCompetitors(
    dto: DomainCompetitorsDto,
  ): Promise<DomainCompetitorsResponse> {
  ...
  private isValidDomain(domain: string): boolean {
    ...
    // Also allow single word domains for testing
    const singleWordRegex = /^[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9]$/;

    return domainRegex.test(domain) || singleWordRegex.test(domain);
  }

  // Method to get domain authority from multiple sources
  async getDomainAuthority(domain: string): Promise<any> {
    ...
    // Mock data based on the API specification
    const mockResponse = {
      domain: domain,
      authorityScore: Math.floor(Math.random() * 100) + 1,
      ahrefsRating: Math.floor(Math.random() * 100) + 1,
      mozScore: Math.floor(Math.random() * 100) + 1,
      semrushScore: Math.floor(Math.random() * 100) + 1,
      majesticScore: Math.floor(Math.random() * 100) + 1,
